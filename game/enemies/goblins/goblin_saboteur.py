...
class GoblinSaboteur(Enemy):
    def __init__(self):
        ...
        self.conditional_ability = {
            "name":"Toxic Flask",
            "status_effects":[
                {"name":"Poison", "duration":3, "amount_per_turn":3},
                {"name":"Accuracy Down", "duration":2}
            ]
        }

    def choose_action(self,player, allies=None):
        # --- 1. CONDITIONAL: TOXIC FLASK (below 30% HP, once per combat) ---
        ...
        if hp_ratio < 0.30 and not self.condition_used:
            ...
            return{
                "name":"Toxic Flask", 
                "skip":True, 
                "status_effects": self.conditional_ability["status_effects"]
            }
        # --- 2. SIGNATURE ABILITY: SHRAPNEL BOMB (weighted + cooldown) ---
